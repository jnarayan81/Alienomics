...
# INPUT FILES --->
  $param{input_genome_fasta} = read_config_file_line('input_genome_fasta', '', $fh_ParamConfig);
...
# PROJECT NAME --->
  $param{output_folder} = read_config_file_line('output_folder', $param{alien_dir}, $fh_ParamConfig);
...
# PROJECT CONFIGURATION --->
  ...
  $param{expected_coverage} = read_config_file_line('expected_coverage', '', $fh_ParamConfig);
 ...
 $param{rejectGi} = read_config_file_line('rejectGi', '', $fh_ParamConfig);
  ...
  $param{clade_self} = read_config_file_line('clade_self', '', $fh_ParamConfig);
  $param{feedlevel_species} = read_config_file_line('feedlevel_species', '', $fh_ParamConfig);
  $param{expected_GC_range} = read_config_file_line('expected_GC_range', '', $fh_ParamConfig);
...
#GENERAL SETTINGS
  ...
  $param{input_genome_gff} = read_config_file_line('input_genome_gff', '', $fh_ParamConfig);
...
# INPUT FILES --->
  if (!defined $param->{input_genome_fasta}) { die ("No path to the nucleotide files was specified in your project's configuration file, please fill the parameter 'input_genome_fasta'.\n"); }
...
# DIAMONDDB FILES --->
  if (!defined $param->{uniref50_path}) { die ("No path to database was found in your project's configuration file, please fill the parameter 'uniref50_path'.\n"); }
  if (!-d $param->{uniref50_path}) { die ("The path to your project's database isn't a valid directory, please check if the path is correct: $param->{uniref50_path}\n"); }
  if (!-r $param->{uniref50_path}) { die ("You don't have permission to read in your diamond protein database, please redefine your permissions.\n"); }
...
if (!defined $param->{augustus_path}) { die ("No path to augustus was specified in SoftConfig at $config_path, please open this file and fill the parameter 'blastn'.\n"); }
  ...
  if (!defined $param->{output_folder}) {die "Project directory not configured. Please set output_folder element in configuration file\n";}

  if (!defined $param->{expected_GC_range}) {die "You must set the parameter \"expected_GC_range\" with an integer ranging from 0 to 100, please check ALIEN documentation form more instructions about this parameter\n";}

  if (($param->{expected_GC_range} !~ /[0-9]/)) {die "You must set the parameter \"expected_GC_range\" with an integer ranging from 0 to 100, please check ALIEN documentation form more instructions about this parameter\n";}
...
sub parse_gene_files {
  ...
  print "\n# Adding gene GC% and expected_coverage info ($date)\n";
  ...
  my (%sequence_data);
    ...
    while (my $gene_info = $file_content->next_seq()) {
      ...
      my $geneCov=0; # Default values for expected_coverage
	if ($param->{expected_coverage} > 0 ) {
...
sub create_conf_file {
  ...
  for (my $i = 0; $i <= $#lines; $i++) {
    my $line = $lines[$i];
    if (($line =~ /^input_genome_fasta/)&&(defined $fasta_path)) {
      $line = "input_genome_fasta = $fasta_path\n";
      $lines[$i] = $line;
    }
    if (($line =~ /^output_folder/)&&(defined $output_dir)) {
      $line = "output_folder = $output_dir\n";
      $lines[$i] = $line;
    }
...
system ("$param->{augustus_path} --species=$param->{species_name_augustus} $genome --outfile=$param->{output_folder}/intermediate_files/gff/preGenes.gff --AUGUSTUS_CONFIG_PATH=$param->{AUGUSTUS_CONFIG_PATH}");
}
